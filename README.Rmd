...
```{r inlasloo, eval = TRUE, echo = TRUE}
      #data
  coords <- data.frame(long = c(rnorm(70), rnorm(30, 3)), lat = rnorm(100))
  PA <- rep(c(0, 1), each = 50)
  x <- data.frame(x1 = rnorm(100),x2 = c(rnorm(70), rnorm(30, 2))) # x1 no relationship, x2 positive relationship
  ...
  modform <- as.formula(paste('y ~ -1+ y.intercept + x1 + f(spatial.field, model=spde)'))#one model
  ...
  alpha <- 0.05#rmse and mae confidence intervals (1-alpha)
      #function
  sloo1 = inlasloo.fun(dataframe=dataframe, long='long', lat='lat',y= 'y', ss=ss, rad=rad, ntrials=ntrials, modform=modform,mesh=mesh,family=family,alpha=0.05,mae=FALSE,ds=FALSE,sqroot=FALSE)
```
