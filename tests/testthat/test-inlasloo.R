...
test_that('x y column names do not break things b.', {
...
test_that('x y column names do not break things c.', {
...
test_that('x y column names do not break things d.', {
  ...
  expect_error(
    # run the function to compare both models
    inl <- inla(modform, data = inla.stack.data(stk, spde = spde), control.predictor = list(A = inla.stack.A(stk), link = 1))
    , NA)
...
test_that('Basic sloo usage works b', {
